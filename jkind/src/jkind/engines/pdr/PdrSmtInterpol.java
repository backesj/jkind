...
public class PdrSmtInterpol extends ScriptUser implements PdrSmt{
	...
	public PdrSmtInterpol(Node node, List<Function> functions, List<Frame> F, String property, String scratchBase) {
	...
	private Term R(int k) {
		...
		for (int i = k; i < F.size(); i++) {
			conjuncts.add(((SmtInterpolFrame)F.get(i)).toTerm(script));
		}
	...
	private SmtInterpolCube extractCube(Model model) {
		if (model == null) {
			return null;
		}

		SmtInterpolCube result = new SmtInterpolCube();
	...
	public boolean isInitial(Cube cube) {
		...
		// contain ~init
		return !((SmtInterpolCube)cube).getPLiterals().contains(not(I));
	}
	...
	public TCube solveRelative(TCube s, Option option) {
		...
		SmtInterpolCube cube = (SmtInterpolCube)s.getCube();
		...
		for (int i = frame - 1; i < F.size() - 1; i++) {
			script.assertTerm(name(((SmtInterpolFrame)F.get(i)).toTerm(script), "F" + i));
		}
		script.assertTerm(((SmtInterpolFrame)F.get(F.size() - 1)).toTerm(script));

		List<Term> pLiterals = ((SmtInterpolCube)s.getCube()).getPLiterals();
		...
		case UNSAT:
			...
			SmtInterpolCube minCube = getMinimalNonInitialCube(pLiterals, unsatCore);
	...
	private SmtInterpolCube getMinimalNonInitialCube(List<Term> pLiterals, Term[] unsatCore) {
		SmtInterpolCube result = new SmtInterpolCube();
		for (Term t : unsatCore) {
			String name = t.toString();
	...
	public void refine(List<Cube> cubes) {
		...
		for (int i = 0; i < cubes.size() - 1; i++) {
			...
			pieces.add(and(apply((SmtInterpolCube)cubes.get(i), vars), T(vars, nextVars)));
			vars = nextVars;
		}
		pieces.add(and(apply((SmtInterpolCube)cubes.get(cubes.size() - 1), vars), not(P(vars))));
	...
	public Expr getInvariant(Cube cube) {
		...
		for (Term literal : ((SmtInterpolCube)cube).getPLiterals()) {
			if (literal != not(I)) {
	...
	private Term[] getVariables(String suffix) {
	...
	private Term not(SmtInterpolCube cube) {
	...
	private Term apply(SmtInterpolCube cube, Term[] arguments) {
	...
	private Term and(SmtInterpolCube cube, Term... terms) {
